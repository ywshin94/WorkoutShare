...
struct CanvasView: View {
    ...
    init(
        ...
        showDateTime: Binding<Bool>, accumulatedOffset: Binding<CGSize>, rotationAngle: Binding<Angle>
    ) {
    ...
    private func clampOffset(potentialOffset: CGSize, canvasSize: CGSize) -> CGSize {
        ...
        let p1 = CGPoint(x: -originalWidth / 2, y: -originalHeight / 2).applying(transform)
        let p2 = CGPoint(x: originalWidth / 2, y: -originalHeight / 2).applying(transform)
        let p3 = CGPoint(x: -originalWidth / 2, y: originalHeight / 2).applying(transform)
        let p4 = CGPoint(x: originalWidth / 2, y: originalHeight / 2).applying(transform)
        ...
        let maxX = canvasSize.width / 2 - (rotatedBlockWidth / 2) - currentBorderMargin
        let minX = -canvasSize.width / 2 + (rotatedBlockWidth / 2) + currentBorderMargin
        let maxY = canvasSize.height / 2 - (rotatedBlockHeight / 2) - currentBorderMargin
        let minY = -canvasSize.height / 2 + (rotatedBlockHeight / 2) + currentBorderMargin
    ...
    private func applyFont(baseSize: CGFloat, weight: Font.Weight = .regular) -> Font {
        ...
        if UIFont(name: selectedFontName, size: scaledSize) != nil {
            ...
            GeometryReader { canvasGeometry in
                ...
                .background(
                    GeometryReader { textBlockGeometry in
                        Color.clear
                            .onAppear { self.textSize = textBlockGeometry.size }
                            .onChange(of: textBlockGeometry.size) { _, newSize in self.textSize = newSize }
                    }
                )
